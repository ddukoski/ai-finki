...
class Beakers(Problem):
    ...
    def successor(self, state):
        ...
        if pfi_r_l is not None:
            successor_states["pour_from_right_to_right"] = pfi_r_l
    ...
    def h(self, node):
        return abs(node.state[0][0] - self.goal[0][0]) + abs(node.state[0][1] - self.goal[1][0])
...
if __name__ == "__main__":
    ...
    """
     ...
     actions and get stuck in a loopy path (unless avoided by DFS loop detection)
    """
